...
class Producto(models.Model):
    ...
    precio_compra_referencial = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    ...
    if created:
        ...
        if instance.cuenta_bancaria:
            ...
            if instance.tipo == 'Ingreso':
                # Al cobrar, el banco te quita el ITF de lo que entra
                obj.saldo_actual += (instance.monto - itf)
            else:
                # Al pagar, sale de tu cuenta el Monto + el ITF
                obj.saldo_actual -= (instance.monto + itf)
            obj.save()
            
        # Lógica para CAJA (Efectivo no tiene ITF)
        elif instance.caja:
            obj = instance.caja
            if instance.tipo == 'Ingreso':
