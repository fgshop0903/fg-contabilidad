                ...
                if destino == 'inventario':
                    ...
                    if prod_id:
                        ...
                        producto = Producto.objects.create(
                            ...
                            stock_actual=0, precio_compra_referencial=0
                        )
...
@login_required
@admin_required # Solo Admins pueden editar montos financieros
def editar_comprobante(request, pk):
    comprobante = get_object_or_404(Comprobante, id=pk, empresa_id=request.session['empresa_id'])
    
    # Creamos una copia del estado actual antes de modificarlo (Para la auditoría)
    comprobante_antes = copy.copy(comprobante)

    if request.method == 'POST':
        nuevo_total = decimal.Decimal(request.POST.get('total'))
        motivo = request.POST.get('motivo_cambio') # Requisito del PDF: Motivo obligatorio
        ...
        # DISPARAMOS LA AUDITORÍA (RNF-02)
        # Esta función comparará 'comprobante_antes' con el nuevo y guardará el JSON
        registrar_auditoria_update(
            usuario=request.user,
            instancia_vieja=comprobante_antes,
            instancia_nueva=comprobante,
            motivo=motivo
        )

        return redirect('lista_comprobantes')

    return render(request, 'core/editar_comprobante.html', {'comprobante': comprobante})
...
def registrar_venta_manual(request):
    ...
    if request.method == 'POST':
        # 1. Cabecera
        dni_ruc = request.POST.get('ruc_dni')
        cliente_nom = request.POST.get('razon_social')
        serie_num = request.POST.get('serie_numero')
        fecha = request.POST.get('fecha')
        moneda = request.POST.get('moneda')
        tc = decimal.Decimal(request.POST.get('tipo_cambio', '1.000'))
        forma_pago = request.POST.get('forma_pago') # 'contado' o 'credito'

        # 2. Cliente
        cliente, _ = Entidad.objects.get_or_create(
            empresa=empresa, numero_documento=dni_ruc,
            defaults={'nombre_razon_social': cliente_nom, 'tipo_entidad': 'Cliente'}
        )

        # 3. Comprobante de Venta
        total_venta = decimal.Decimal('0.00')
        venta = Comprobante.objects.create(
            empresa=empresa, entidad=cliente,
            tipo_documento='Recibo', operacion='Venta',
            serie='V-MAN', numero=serie_num,
            fecha_emision=fecha, moneda=moneda, tipo_cambio=tc,
            subtotal=0, igv=0, total=0, estado_sunat='INTERNO'
        )
        ...
        for i in range(len(descripciones)):
            cant = decimal.Decimal(cantidades[i])
            prec = decimal.Decimal(precios[i])
            subtotal_fila = cant * prec
            total_venta += subtotal_fila
            
            producto = get_object_or_404(Producto, id=prod_ids[i], empresa=empresa)
        ...
        venta.igv = decimal.Decimal('0.00')
        venta.total = total_venta
        venta.save()

        # 6. Lógica de Pago / Crédito
        cuenta = CuentaEstado.objects.create(
            comprobante=venta, monto_total=total_venta,
            saldo_pendiente=total_venta, 
            fecha_vencimiento=datetime.date.today()
        )

        if forma_pago == 'contado':
            # Si es contado, registramos el ingreso a caja de una vez
            caja_id = request.POST.get('caja_id')
            banco_id = request.POST.get('banco_id')
            
            MovimientoFinanciero.objects.create(
                empresa=empresa, tipo='Ingreso', monto=total_venta,
                moneda=moneda, referencia=f"Pago Contado Venta Manual {serie_num}",
                comprobante=venta, caja_id=caja_id if caja_id else None,
                cuenta_bancaria_id=banco_id if banco_id else None
            )
            cuenta.saldo_pendiente = 0
            cuenta.estado = 'Cancelado'
            ...
            # Si es crédito, lo mandamos a programar las cuotas que ya tenemos hecho
            return redirect('configurar_cuotas', cuenta_id=cuenta.id)
