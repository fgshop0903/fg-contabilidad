...
def registrar_auditoria_update(usuario, instancia_vieja, instancia_nueva, motivo):
    """
    RNF-02: Registra qué cambió exactamente en una operación financiera.
    """
    cambios = []
    dict_viejo = model_to_dict(instancia_vieja)
    dict_nuevo = model_to_dict(instancia_nueva)

    # Comparamos el total, que es lo más crítico
    if instancia_vieja.total != instancia_nueva.total:
        cambios.append(f"Total: de {instancia_vieja.total} a {instancia_nueva.total}")


    for campo, valor in dict_viejo.items():
        if valor != dict_nuevo.get(campo):
            cambios[campo] = {
                'antes': str(valor),
                'despues': str(dict_nuevo.get(campo))
            }
    ...
    if cambios:
        ...
        LogAuditoria.objects.create(
            ...
            motivo_cambio=f"{' | '.join(cambios)} | Motivo: {motivo}"
        )
